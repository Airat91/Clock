...
void hd44780_send (uint8_t data, cmd_dat_type cmd_dat); // Функция отправки данных в дисплей: CMD - команда, DAT - данные
void hd44780_xy (uint8_t x, uint8_t y);            // Функция установки курсора в позицию х(счет от 1до 16) строки у(счет от 1 до 2)
void hd44780_string (uint8_t string[], uint16_t ms); // Функция вывода на экран строки STRING, после вывода каждого символа происходит задержка MS миллисекунд
...
void hd44780_user_symbol (uint8_t adr, const uint8_t symbol[8]);  // Функция записи пользовательского символа 5х8: ADR - адрес ячейки CGRAM, SYMBOL - массив из 8 строк вида uint8_t SYMBOL[8]={
                                                // 0b000XXXXX,          
                                                // 0b000XXXXX,
                                                // 0b000XXXXX,
   //        ВАЖНО!!!                           // 0b000XXXXX,
...
void hd44780_shift (left_right_type left_righ, uint8_t num, int8_t ms);      // Функция сдвига экрана: LEFT_RIGHT=LEFT - влево, LEFT_RIGHT=RIGHT - вправо, NUM - количество позиций сдвига, MS - задержка после сдвига каждого символа
//void hd44780_delay(int MS);                         // Функция задержки в миллисекундах
void hd44780_num (int32_t num);                          // Функция вывода на экран числа с учетом знака. Число пишется влево от текущей позиции курсора
